...
def getAction_termux(_as=None,_data=None):
  def clip(__data):
    ...
    resp = termux_api.confirm("继续？")
    return ((resp["code"]and resp["back"])and getAction_termux())or exit()
  if not _as:
    resp = termux_api.sheet(["in","de"],"in/de")
    return getAction_termux((resp["code"]and["in","de"][resp["back"]])or exit(),_data)
  if not _data:
    resp = termux_api.text("操作字符","输入","m")
    return getAction_termux(_as,(resp["code"]and resp["back"])or exit())
  if _as=="de":
    clip(outfurr(_data,[["喵", "嗷"], ["呜"], ["~"], ["咪"]]))
  else:
    clip(infurr(_data,[["喵", "嗷"], ["呜"], ["~"], ["咪"]]))
  return None
  """def clip(__data):
    print(__data)
    termux_api.clipbard_set(__data)
